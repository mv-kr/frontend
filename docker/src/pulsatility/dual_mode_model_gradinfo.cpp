...
const std::string mci::models::dual_mode_model_gradinfo::PARAM_dt = "dt";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_B_1 = "B_1";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_mu_tau_on =  "mu_tau_on";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_mu_tau_off = "mu_tau_off";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_std_tau_on = "std_tau_on";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_std_tau_off ="std_tau_off";

const std::string mci::models::dual_mode_model_gradinfo::PARAM_f = "f";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_alpha_1 = "alpha_1";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_alpha_2 = "alpha_2";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_tau_1 = "tau_1";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_tau_2 = "tau_2";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_LH_0 = "LH_0";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_sigma_obs = "sigma_obs";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_observed_variable_index_0 = "observed_variable_index_0";
...
const std::string mci::models::dual_mode_model_gradinfo::NAME = "dual_mode_model_gradinfo";

const std::string mci::models::dual_mode_model_gradinfo::PARAM_prior_mean_B_1 = "prior_mean_B_1";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_prior_std_B_1 = "prior_std_B_1";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_prior_mean_alpha_1 = "prior_mean_alpha_1";
const std::string mci::models::dual_mode_model_gradinfo::PARAM_prior_std_alpha_1 = "prior_std_alpha_1";
/** constructor **/
dual_mode_model_gradinfo::dual_mode_model_gradinfo(const model_params_t& p, const mci::stochastic& r, const std::vector<std::string>& pi) {
    params = p;
    rng = r;
    variables_num = params[dual_mode_model_gradinfo::PARAM_dim];
    obs_variable_index(0) = params[dual_mode_model_gradinfo::PARAM_observed_variable_index_0];
    iparam_names = pi;
    
    params[PARAM_grad_dim] = 3;
    
}

/** Get all model parameters **/
const model_params_t dual_mode_model_gradinfo::get_parameters() {
    return params;
}

/** Get parameter with identifier str **/
const double dual_mode_model_gradinfo::get_parameter(const std::string& str) {
    return params[str];
}

const size_t dual_mode_model_gradinfo::get_dim() {
    return this->variables_num;
}

const double dual_mode_model_gradinfo::get_dt() {
    return this->params[dual_mode_model_gradinfo::PARAM_dt];
}


/** Set the model parameters **/
void dual_mode_model_gradinfo::set_parameters(const model_params_t& p) {
    params = p;
}


/** return logarithm of parameter prior density \pi(theta) **/
const double dual_mode_model_gradinfo::log_prior_density(const Eigen::VectorXd& theta) {
    ...
    val = - (theta(3) + theta(4))*log(10)
    //- theta(3) - 2*log(1 + exp(-theta(3)) )
    //- theta(4) - 2*log(1 + exp(-theta(4)) )
    // - 1./(2.*pow(0.25,2))*pow( theta(2) - 1.,2)
    // - 1./(2.*pow(0.3,2))*pow( theta(3) - 1.5,2)
    //- 1./(2.*pow(0.3,2))*pow( theta(0) - 2.6,2)
    - 1./(2.*pow(params[PARAM_prior_std_B_1],2))*pow( theta(0) - params[PARAM_prior_mean_B_1],2)
    ...
    - 1./(2.*pow(9.3,2))*pow( log(2.)/pow(10.,theta(1)) - 80.0,2)-theta(1)*log(10.)
...
/** return logarithm of parameter prior density \pi(theta) **/
const Eigen::VectorXd dual_mode_model_gradinfo::grad_log_prior_density(const Eigen::VectorXd& theta) {
        ...
        prior_grad(1) = log(10.) * (-1 + 1./(pow(9.3,2))*log(2.)/pow(10,theta(1))*( log(2.)/pow(10,theta(1)) - 80.0) );//- 1./(pow(0.3,2))*( theta(1) + 2.0623); // clearance
        prior_grad(2) = - 1 + 2./(1+exp(theta(2))) ;
...
const void dual_mode_model_gradinfo::add_prior_information(const Eigen::VectorXd& theta, Eigen::MatrixXd* G, std::vector<Eigen::MatrixXd>* G_k) {
    ...
    (*G)(1,1) -=  log(2)*log(10.)*log(10.)/(pow(9.3,2)) * ( 80*pow(10,-theta(1)) - log(4)*pow(10,-2*theta(1)) );
    (*G)(2,2) -=  - exp(theta(2)) * 2./pow(1+exp(theta(2)),2);
...
const double dual_mode_model_gradinfo::log_proposal_density(const Eigen::VectorXd& theta_current, const Eigen::VectorXd& theta_proposed) {
    return 0.0;
}

/** update the inferred model parameters**/
void dual_mode_model_gradinfo::update_i_parameters(const Eigen::VectorXd& pp) {
...
const Eigen::VectorXd dual_mode_model_gradinfo::get_i_parameters() {
...
const double dual_mode_model_gradinfo::log_obs_density( const state_t & X, const Eigen::VectorXd& Y) {
    
    double sigma_obs = params[dual_mode_model_gradinfo::PARAM_sigma_obs];
...
Eigen::MatrixXd dual_mode_model_gradinfo::FIM( const state_t & X) {
    
    size_t N = params[PARAM_grad_dim];
    Eigen::MatrixXd FIM = Eigen::VectorXd::Zero(N);
    Eigen::VectorXd grad = Eigen::VectorXd::Zero(N);
    double alpha_1 = params[dual_mode_model_gradinfo::PARAM_alpha_1];
    double B_1 = params[dual_mode_model_gradinfo::PARAM_B_1];
    double f = params[dual_mode_model_gradinfo::PARAM_f];
    double sigma_obs = params[dual_mode_model_gradinfo::PARAM_sigma_obs];
...
/** return gradient of the logarithm of conditional observation density g(X_t,Y_t) with respect to the parameters**/
const Eigen::VectorXd dual_mode_model_gradinfo::grad_log_obs_density( const state_t & X, const Eigen::VectorXd& Y) {
...
const double dual_mode_model_gradinfo::log_transition_density(const state_t& X_t2, const state_t& X_t1, const size_t& t) {
...
const double dual_mode_model_gradinfo::log_joint_density(const trajectory_t& X_ref_traj, const trajectory_t& data, double* ll) {
    
    double dt = params[dual_mode_model_gradinfo::PARAM_dt];
...
const Eigen::VectorXd dual_mode_model_gradinfo::grad_log_joint_density(const trajectory_t& X_ref_traj, const trajectory_t& data,double* jll, double* mll, Eigen::MatrixXd *FIM_ret, std::vector<Eigen::MatrixXd>* G_k) {
...
const state_t dual_mode_model_gradinfo::generate_ic(const double t_0) {
    
    double dt           = params[dual_mode_model_gradinfo::PARAM_dt];
    double mu_tau_on    = params[dual_mode_model_gradinfo::PARAM_mu_tau_on] ;
    double mu_tau_off   = params[dual_mode_model_gradinfo::PARAM_mu_tau_off] ;
    double std_tau_on   = params[dual_mode_model_gradinfo::PARAM_std_tau_on] ;
    double std_tau_off  = params[dual_mode_model_gradinfo::PARAM_std_tau_off] ;
    ...
    double tau_1 = params[dual_mode_model_gradinfo::PARAM_tau_1];
    double B_1 = params[dual_mode_model_gradinfo::PARAM_B_1] ;
    double alpha_1 = params[dual_mode_model_gradinfo::PARAM_alpha_1];
    double LH_0         = params[dual_mode_model_gradinfo::PARAM_LH_0];
    double f = params[dual_mode_model_gradinfo::PARAM_f];//1./(exp(params[dual_mode_model_gradinfo::PARAM_f])+1);
...
const trajectory_t dual_mode_model_gradinfo::run(const state_t& X0, const double& t_start, const double& t_end, const trajectory_t& data) {
    ...
    double std_tau_off  = params[dual_mode_model_gradinfo::PARAM_std_tau_off] ;
    double tau_1 = params[dual_mode_model_gradinfo::PARAM_tau_1];
    double tau_2 = params[dual_mode_model_gradinfo::PARAM_tau_2];
...
const trajectory_t dual_mode_model_gradinfo::run(const trajectory_t& X_ref_traj, const state_t& X0, const trajectory_t& data) {
    ...
    double B_1 = params[dual_mode_model_gradinfo::PARAM_B_1] ;
    double alpha_1 = params[dual_mode_model_gradinfo::PARAM_alpha_1];
    double alpha_2 = params[dual_mode_model_gradinfo::PARAM_alpha_2];
    double f = params[dual_mode_model_gradinfo::PARAM_f];
    double LH_0         = params[dual_mode_model_gradinfo::PARAM_LH_0];
...
double dual_mode_model_gradinfo::propensity(const double x, const double m, const double s) {
...
double dual_mode_model_gradinfo::get_quantile(const double x, const double m, const double s) {
    double dt = params[dual_mode_model_gradinfo::PARAM_dt];
...
double dual_mode_model_gradinfo::propensity_on(const double x, const double m, const double s) {
...
double dual_mode_model_gradinfo::get_quantile_on(const double x, const double m, const double s) {
    double dt = params[dual_mode_model_gradinfo::PARAM_dt];
