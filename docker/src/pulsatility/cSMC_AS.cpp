    ...
    for (size_t ti = 1; ti<obs_timepoints; ti++ ) {
        ...
        if (rng.rand() <= as_prob) {
            ...
            for (unsigned int pi = 0; pi< nparticles; pi++) {
                ...
                //std::cout<<" model_t_index "<<model_t_index << " Xfinal.size()" << Xfinal << std::endl;
                // NOTE WHICH IS THE CORRECT, REQUIRES ATTENTION
                for (size_t kk = model_t_index+1; kk<Xfinal.cols(); kk++)
                    WW_as(pi) = WW_as(pi) + model_obj->log_transition_density(Xfinal.col(kk), Xfinal.col(kk-1), kk);
...
/** run the algorithm and store results in res_traj **/
const std::vector<Eigen::MatrixXd> cSMC_AS::run_pop(const trajectory_t& ref_traj,
    ...
    size_t model_timepoints = ref_traj.cols();
    // number of particles
    unsigned int nparticles = pop_size;//parameters[cSMC_AS::PARAM_nparticles];
    // AS probability
    double as_prob = parameters[cSMC_AS::PARAM_AS_probability];
    ...
    // sample trajectory
    std::vector<Eigen::MatrixXd> particles_sample(sample_size) ;
    
    
    WW = weights.col(obs_timepoints-1);
    WW.array() -= WW.maxCoeff();
    unsigned int pi;
    for (size_t ii=0; ii< sample_size; ii++) {
        pi = rng.rand_sample(WW.array().exp()) ;
        //res_traj.resize(model_obj->get_dim(), model_timepoints);
        particles_sample[ii] = particles[pi];
    }
