...
const std::string mci::models::LH_secretion_model::PARAM_dt = "dt";
const std::string mci::models::LH_secretion_model::PARAM_B_1 = "k";
const std::string mci::models::LH_secretion_model::PARAM_mu_tau_on =  "mu_tau_on";
const std::string mci::models::LH_secretion_model::PARAM_mu_tau_off = "mu_tau_off";
const std::string mci::models::LH_secretion_model::PARAM_std_tau_on = "std_tau_on";
const std::string mci::models::LH_secretion_model::PARAM_std_tau_off ="std_tau_off";

const std::string mci::models::LH_secretion_model::PARAM_f = "f";
const std::string mci::models::LH_secretion_model::PARAM_alpha_1 = "d";
const std::string mci::models::LH_secretion_model::PARAM_tau_1 = "tau_1";
const std::string mci::models::LH_secretion_model::PARAM_LH_0 = "LH_0";
const std::string mci::models::LH_secretion_model::PARAM_sigma_obs = "sigma_obs";
const std::string mci::models::LH_secretion_model::PARAM_observed_variable_index_0 = "observed_variable_index_0";
...
const std::string mci::models::LH_secretion_model::NAME = "LH_secretion_model";

const std::string mci::models::LH_secretion_model::PARAM_prior_mean_B_1 = "k_normal_prior_mean";
const std::string mci::models::LH_secretion_model::PARAM_prior_std_B_1 = "k_normal_prior_std";
const std::string mci::models::LH_secretion_model::PARAM_prior_mean_alpha_1 = "d_normal_prior_mean";
const std::string mci::models::LH_secretion_model::PARAM_prior_std_alpha_1 = "d_normal_prior_std";
const std::string mci::models::LH_secretion_model::PARAM_f_Beta_prior_alpha = "f_Beta_prior_alpha";
const std::string mci::models::LH_secretion_model::PARAM_f_Beta_prior_beta = "f_Beta_prior_beta";
/** constructor **/
LH_secretion_model::LH_secretion_model(const model_params_t& p, const mci::stochastic& r, const std::vector<std::string>& pi) {
    params = p;
    rng = r;
    variables_num = 23; //params[dual_mode_model_gradinfo::PARAM_dim];
    grad_num = 3;
    obs_variable_index(0) = params[LH_secretion_model::PARAM_observed_variable_index_0];
    ...
    std::cout << iparam_id[0] << " " << iparam_id[1] <<std::endl;
    
    
}

/** Get all model parameters **/
const model_params_t LH_secretion_model::get_parameters() {
    return params;
}

/** Get parameter with identifier str **/
const double LH_secretion_model::get_parameter(const std::string& str) {
    return params[str];
}

const size_t LH_secretion_model::get_dim() {
    return this->variables_num;
}

const double LH_secretion_model::get_dt() {
    return this->params[LH_secretion_model::PARAM_dt];
}


/** Set the model parameters **/
void LH_secretion_model::set_parameters(const model_params_t& p) {
    params = p;
}


/** return logarithm of parameter prior density \pi(theta) **/
const double LH_secretion_model::log_prior_density(const Eigen::VectorXd& theta) {
...
const Eigen::VectorXd LH_secretion_model::grad_log_prior_density(const Eigen::VectorXd& theta) {
    
        size_t N = grad_num;
        Eigen::VectorXd prior_grad = Eigen::VectorXd::Zero(N);
        

        prior_grad(iparam_id[0]) = - 1./(pow(params[PARAM_prior_std_B_1],2))*( theta(iparam_id[0]) - params[PARAM_prior_mean_B_1]);
        prior_grad(iparam_id[1]) = log(10.) * (-1 + 1./(pow(params[PARAM_prior_std_alpha_1],2))*log(2.)/pow(10,theta(iparam_id[1]))*( log(2.)/pow(10,theta(iparam_id[1])) - params[PARAM_prior_mean_alpha_1]) ); // clearance
        ...
        prior_grad(iparam_id[2]) = - 1 + 2./(1+exp(theta(iparam_id[2])))
...
const void LH_secretion_model::add_prior_information(const Eigen::VectorXd& theta, Eigen::MatrixXd* G, std::vector<Eigen::MatrixXd>* G_k) {
    
    size_t N = grad_num;
    Eigen::VectorXd prior_grad = Eigen::VectorXd::Zero(N);
    
    
    (*G)(iparam_id[0],iparam_id[0]) -=  - 1./(pow(params[PARAM_prior_std_B_1],2));
    (*G)(iparam_id[1],iparam_id[1]) -=  log(2)*log(10.)*log(10.)/(pow(params[PARAM_prior_std_alpha_1],2)) * ( params[PARAM_prior_mean_alpha_1]*pow(10,-theta(iparam_id[1])) - log(4)*pow(10,-2*theta(iparam_id[1]) ) );
    ...
    (*G)(iparam_id[2],iparam_id[2]) -=  - 2. * exp(theta(iparam_id[2])) /pow(1+exp(theta(iparam_id[2])),2)
...
const double LH_secretion_model::log_proposal_density(const Eigen::VectorXd& theta_current, const Eigen::VectorXd& theta_proposed) {
    return 0.0;
}

/** update the inferred model parameters**/
void LH_secretion_model::update_i_parameters(const Eigen::VectorXd& pp) {
...
const Eigen::VectorXd LH_secretion_model::get_i_parameters() {
...
const double LH_secretion_model::log_obs_density( const state_t & X, const Eigen::VectorXd& Y) {
    
    double sigma_obs = params[LH_secretion_model::PARAM_sigma_obs];
...
Eigen::MatrixXd LH_secretion_model::FIM( const state_t & X) {
    
    size_t N = grad_num;
    Eigen::MatrixXd FIM = Eigen::VectorXd::Zero(N);
    Eigen::VectorXd grad = Eigen::VectorXd::Zero(N);
    double alpha_1 = params[LH_secretion_model::PARAM_alpha_1];
    double B_1 = params[LH_secretion_model::PARAM_B_1];
    double f = params[LH_secretion_model::PARAM_f];
    double sigma_obs = params[LH_secretion_model::PARAM_sigma_obs];
    
    grad(iparam_id[0]) =  X(18,0) * log(10) * B_1;
    grad(iparam_id[1]) =  X(19,0) * log(10) * alpha_1;
    grad(iparam_id[2]) =  X(21,0) * f * (1-f);
...
/** return gradient of the logarithm of conditional observation density g(X_t,Y_t) with respect to the parameters**/
const Eigen::VectorXd LH_secretion_model::grad_log_obs_density( const state_t & X, const Eigen::VectorXd& Y) {
    ...
    */
    ...
    double s_LH  = sigma_obs*abs(X(obs_variable_index(0),0)) ;
    grad(iparam_id[0]) =  - 1./s_LH*sigma_obs*X(18,0) * log(10) * B_1
    ...
    grad(iparam_id[1]) = - 1./s_LH*sigma_obs*X(19,0) * log(10) * log(10) * alpha_1
    ...
    grad(iparam_id[2]) = - 1./s_LH*sigma_obs*X(21,0) * f * (1-f)
...
const double LH_secretion_model::log_transition_density(const state_t& X_t2, const state_t& X_t1, const size_t& t) {
...
const double LH_secretion_model::log_joint_density(const trajectory_t& X_ref_traj, const trajectory_t& data, double* ll) {
    
    double dt = params[LH_secretion_model::PARAM_dt];
...
const Eigen::VectorXd LH_secretion_model::grad_log_joint_density(const trajectory_t& X_ref_traj, const trajectory_t& data,double* jll, double* mll, Eigen::MatrixXd *FIM_ret, std::vector<Eigen::MatrixXd>* G_k) {
...
const state_t LH_secretion_model::generate_ic(const double t_0) {
    
    double dt           = params[LH_secretion_model::PARAM_dt];
    double mu_tau_on    = params[LH_secretion_model::PARAM_mu_tau_on] ;
    double mu_tau_off   = params[LH_secretion_model::PARAM_mu_tau_off] ;
    double std_tau_on   = params[LH_secretion_model::PARAM_std_tau_on] ;
    double std_tau_off  = params[LH_secretion_model::PARAM_std_tau_off] ;
    ...
    double tau_1 = params[LH_secretion_model::PARAM_tau_1];
    double B_1 = params[LH_secretion_model::PARAM_B_1] ;
    double alpha_1 = params[LH_secretion_model::PARAM_alpha_1];
    double LH_0         = params[LH_secretion_model::PARAM_LH_0];
    double f = params[LH_secretion_model::PARAM_f];//1./(exp(params[dual_mode_model_gradinfo::PARAM_f])+1);
...
const trajectory_t LH_secretion_model::run(const state_t& X0, const double& t_start, const double& t_end, const trajectory_t& data) {
...
const trajectory_t LH_secretion_model::run(const trajectory_t& X_ref_traj, const state_t& X0, const trajectory_t& data) {
...
double LH_secretion_model::propensity(const double x, const double m, const double s) {
...
double LH_secretion_model::get_quantile(const double x, const double m, const double s) {
    double dt = params[LH_secretion_model::PARAM_dt];
...
double LH_secretion_model::propensity_on(const double x, const double m, const double s) {
...
double LH_secretion_model::get_quantile_on(const double x, const double m, const double s) {
