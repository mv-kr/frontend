...
const void shMCMC_gibbs_sMMALA::run(chain_doubles_t& ch) {
    ...
    Eigen::MatrixXd X_sample_best(model_state_dim, model_timepoints);
    ...
    for(size_t ir=0; ir<adapt_runs; ir++) {
        ...
        for (unsigned int i=1 ; i<iter_adapt; i++) {
            ...
            if ( (i+1)%(iter_adapt/10)  == 0 || i+1 == iter_adapt ){
                ...
                std::cout << "(thread " << OpenMP::getThreadNum() << ") --- adapt run " << ir+1<< "/" << adapt_runs << ", iteration " << i+1 << "/" << (iter_adapt)
                << " (time " << time
...
const double shMCMC_gibbs_sMMALA::MH_sampler_single_parameter(Eigen::VectorXd& theta,
    ...
    acceptance_prob = 0.0;
    double ret_value = joint_llik + model_ptr->log_prior_density(theta_current);
    if ( ! cond  ) {
        ...
        double p_tmp= model_ptr->log_prior_density(theta_star);
        /* calculate the acceptance ratio */
        double log_prior_ratio = p_tmp
        ...
        if (std::isnan(acceptance_ratio)) {
            ...
            if (rng.rand() < acceptance_prob) {
               ...
               ret_value = joint_Lik_star + p_tmp;
            }
        }
